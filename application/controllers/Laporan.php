...
class Laporan extends CI_Controller
{
    ...
    {
        $datalog = $this->db->get('tb_reprint')->result();
        foreach ($datalog as $row) {
            ...
            $pdf->MultiCell(95, ($cellHeight), $row->no_mt_rep, 0, 'L');

            //tinggi cell maksud perjalanan
            $cellWidth = 95;
            $cellHeight = 7;
            if ($pdf->GetStringWidth($row->petugas2_rep) < $cellWidth) {
                $line = 1;
            } else {
                $textLength = strlen($row->petugas2_rep);    //total panjang teks
                ...
                while ($startChar < $textLength) { //perulangan sampai akhir teks
                    ...
                    while (
                        ...
                        $maxChar++;
                        $tmpString = substr($row->petugas2_rep, $startChar, $maxChar);
                    }
            ...
            $pdf->Cell(95, ($cellHeight), $row->petugas1_rep, 0, 0);

            $pdf->Cell(10, ($cellHeight), '', 0, 1); //spasi
            $pdf->Cell(40, ($cellHeight), '', 0, 0);
            $pdf->Cell(3, ($cellHeight), '', 0, 0);
            $pdf->Cell(5, ($cellHeight), '2.', 0, 0);
            $pdf->Cell(95, ($cellHeight), $row->petugas2_rep, 0, 0);

            $pdf->Cell(10, (3), '', 0, 1); //spasi
            $pdf->Cell(10, ($cellHeight), '', 0, 1); //spasi
            $pdf->Cell(40, ($cellHeight), 'Nama Barang', 0, 0);
            $pdf->Cell(3, ($cellHeight), ':', 0, 0);
            $pdf->Cell(95, ($cellHeight), $row->nama_barang_rep, 0, 0);

            $pdf->Cell(10, ($cellHeight), '', 0, 1); //spasi
            $pdf->Cell(40, ($cellHeight), 'Kode Barang', 0, 0);
            $pdf->Cell(3, ($cellHeight), ':', 0, 0);
            $pdf->Cell(95, ($cellHeight), $row->kode_barang_rep, 0, 0);
            ...
            $pdf->MultiCell(140, ($cellHeight), $row->deskripsi_rep, 0, 'J');
        }

        $pdf->Output('D', 'Logbook Perawatan Alat.pdf');

        // redirect('datadata/log');
